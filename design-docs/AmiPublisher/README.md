...
- Concurrently instruct all *ImageUnpacker* servers to unpack the image
- Wait for image to be replicated to all regions (optional, typically < 10 seconds)
- Wait for volume to be updated by the *ImageUnpacker* (typically < 10 seconds)
...
The following diagram shows an *ImageUnpacker* instance. Each image stream is pinned to a particular volume. The *ImageUnpacker* tracks these relationships. It has no knowledge of AWS resources or APIs. It simply deals in image content and block devices attached to it.
...
- Discovers *ImageUnpacker* instances automatically (by searching for instances with the “Name” tag having the `ImageUnpacker` value)
- Creates volumes as needed
- Attaches volumes to *ImageUnpacker* instances
- Co-ordinates the *ImageUnpacker*s
- Snapshots volumes
...
However, if an *ImageUnpacker* instance is not likely to be busy, then a cheaper t2.medium instance ($34/month) will be sufficient, with no measurable increase in latency.
...
- Testing branch: a faster, more expensive *ImageUnpacker* instance can be configured in the same target where the image will be tested
- General branch: the remaining *ImageUnpacker* instances can be slower. By the time the image is tested, these slower *ImageUnpacker* instances should have finished, and thus there is no latency penalty from using slower instances.

### Stop most *Image Unpacker* instances

This is a more aggressive extension of the previous cost optimisation. If the time taken to perform image testing is more than a few minutes (see below), then *ImageUnpacker* instances in the general branch can be stopped while idle, and started on-demand when an image build is started. Because stopped instances do not incur rental costs (aside from attached volumes, which have negligible cost), it is viable to have only one long-running *ImageUnpacker* instance and the rest are stopped most of the time.
...
Note that the *[AMI Publisher](../../cmd/ami-publisher/README.md)* will automatically start an *ImageUnpacker* instance which is stopped.

The diagram above shows the *ImageUnpacker* instance on the testing branch in bold. The diagram below shows the image building workflow described in this section.

![AMI Building Pipeline image](../pictures/AmiBuildingPipeline.svg)

This shows the dependency flow between the various stages. Events that occur at approximately the same time are vertically aligned, with time flowing from left to right. If an *ImageUnpacker* instance is stopped it will be started during the prepare stage.
...
## The First *ImageUnpacker* instance
...
If encrypted EBS-backed “base” images are already available in your AWS account (such as when you already have an image build system and wish to migrate to the image unpacking system), then the simplest approach is to launch an instance using one of these images and installing the `image-unpacker` binary and SSL key on the instance. You now have your first *ImageUnpacker* instance. Alternatively, instead of copying over the `image-unpacker` binary you can set the `RequiredImage` tag to specify a [Dominator](../Dominator/README.md) image which contains the `image-unpacker` binary and configuration. The [Dominator](../Dominator/README.md) will automatically convert the new instance into a running *ImageUnpacker*. See below for a rationale for this approach.
...
We desire a more automated solution that minimises account administrator involvement. The *[AMI Publisher](../../cmd/ami-publisher/README.md)* can be used to bootstrap *ImageUnpacker* instances into new targets, once you have your first *ImageUnpacker* and a base image it has published, somewhere in AWS. The steps are fully automated and do not require any account administrator involvement (other than being given API access keys for the accounts you wish to publish images into). The *[AMI Publisher](../../cmd/ami-publisher/README.md)* can bootstrap *ImageUnpacker* instances into multiple (all) accounts and regions concurrently with a single command and can be re-run to bootstrap *ImageUnpacker* instances in new regions and accounts as they become available.
...
- Find an existing nearest *ImageUnpacker* instance with one of your unpacked base images
...
- Launch a second instance (which will become the permanent *ImageUnpacker* instance for that target) with the newly registered base image and set the `RequiredImage` tag to specify a [Dominator](../Dominator/README.md) image which contains the `image-unpacker` binary and configuration. The [Dominator](../Dominator/README.md) will automatically convert the new instance into a running *ImageUnpacker*.

This approach leverages the same system (the [Dominator](../Dominator/README.md)) which is used to update *ImageUnpacker* instances (and other infrastructure that is not suited to the container paradigm) to bootstrap from a base image to a functioning *ImageUnpacker*. By using the nearest existing *ImageUnpacker* instance to copy the image from, cross-region traffic can be minimised.

A further optimisation is that the *[AMI Publisher](../../cmd/ami-publisher/README.md)* will skip the steps required to register a base image if one already exists in a target. This is particularly helpful if *ImageUnpacker* instances are destroyed or if migrating into multiple targets (where you already have base images) to the image unpacking system.
...
- Only one *ImageUnpacker* instance per region is required
- The `ec2-bundle-image` and `ec2-upload-bundle` utilities from AWS are required on the *ImageUnpacker* instances (this requires credentials to upload to S3 buckets)
