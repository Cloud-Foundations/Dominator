...
// BuildMissingLists returns a map of objects and their corresponding lengths to
// fetch and a map of files to push. The map is nil if there are missing
// computed files.
func BuildMissingLists(sub Sub, img *image.Image, pushComputedFiles bool,
	...
	return sub.buildMissingLists(img, nil, pushComputedFiles,
		ignoreMissingComputedFiles, logger)
}

// BuildBothMissingLists will construct lists of objects to be fetched by the
// sub from an object server and the list of computed objects that should be
// pushed to the sub. The lists are generated by comparing the contents of
// sub.FileSystem and sub.ObjectCache with the required and planned images.
// If pushComputedFiles is true then the list of computed files to be pushed is
// generated.
// If ignoreMissingComputedFiles is true then missing computed files are
// ignored, otherwise these missing files lead to an error and early termination
// of the function.
// Computed file metadata are specified by sub.ComputedInodes.
// BuildBothMissingLists returns a map of objects and their corresponding
// lengths to fetch and a map of files to push. The map is nil if there are
// missing computed files.
func BuildBothMissingLists(sub Sub, requiredImage, plannedImage *image.Image,
	pushComputedFiles, ignoreMissingComputedFiles bool, logger log.Logger) (
	map[hash.Hash]uint64, map[hash.Hash]struct{}) {
	return sub.buildMissingLists(requiredImage, plannedImage, pushComputedFiles,
		ignoreMissingComputedFiles, logger)
}
