...
func (m *Manager) createVm(conn *srpc.Conn) error {
	...
	if request.UserDataSize > 0 {
		...
		err := copyData(filename, conn, request.UserDataSize, vm.logger, m.DisableFillZero)
		if err != nil {
			return sendError(conn, err)
		}
	}
	if len(request.SecondaryVolumes) > 0 {
		...
		for index, volume := range request.SecondaryVolumes {
			...
			err := copyData(fname, dataReader, volume.Size, vm.logger, m.DisableFillZero)
			if err != nil {
				return sendError(conn, err)
			}
...
func (m *Manager) debugVmImage(conn *srpc.Conn,
	...
	if request.ImageName != "" {
		...
		err := copyData(rootFilename, conn, request.ImageDataSize, vm.logger, m.DisableFillZero)
		if err != nil {
			return sendError(conn, err)
		}
	} else if request.ImageURL != "" {
		...
		err = copyData(rootFilename, httpResponse.Body,
			uint64(httpResponse.ContentLength), vm.logger, m.DisableFillZero)
		if err != nil {
			return sendError(conn, err)
		}
	} else {
...
func (m *Manager) replaceVmImage(conn *srpc.Conn,
	...
	if request.ImageName != "" {
		...
		err := copyData(tmpRootFilename, conn, request.ImageDataSize, vm.logger, m.DisableFillZero)
		if err != nil {
			return err
		}
...
func (vm *vmInfoType) copyRootVolume(request proto.CreateVmRequest,
	...
	err = copyData(vm.VolumeLocations[0].Filename, reader, dataSize, vm.logger, false)
	if err != nil {
		return err
	}
